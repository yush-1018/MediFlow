...
class AdminIndentApprovalPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminIndentApprovalPage> createState() => _AdminIndentApprovalPageState();
}

class _AdminIndentApprovalPageState extends ConsumerState<AdminIndentApprovalPage> {
  ...
  Future<void> _analyzeRequest(MedRequest request) async {
    ...
    try {
      ...
      final inventory = await firebaseService.getInventoryOnce(request.facilityId);
      final logs = await firebaseService.getRecentLogs(request.facilityId, days: 90);
      ...
      final forecast = await aiService.forecastDemand(request.medicineName, logs, 30);
      final predictedDemand = forecast['prediction'] as int;

      String suggestion;
      if (request.quantity > (predictedDemand * 1.5)) {
        suggestion = '⚠️ REDUCE: Request is 50%+ higher than predicted 30-day demand ($predictedDemand).';
      } else if (currentItem.remainingQuantity > predictedDemand) {
        suggestion = '⚠️ DECLINE: Facility already has enough stock (${currentItem.remainingQuantity}) for predicted demand ($predictedDemand).';
      } else {
        suggestion = '✅ APPROVE: Request is aligned with historical usage and current low stock.';
      }
  ...
  Future<void> _updateStatus(String requestId, RequestStatus status) async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Request ${status.name} successfully!')));
    } catch (e) {
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(color: MediColors.surfaceLight, borderRadius: BorderRadius.circular(8)),
                            child: Text('${req.quantity} Units', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: MediColors.textPrimary)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 16),
                      if (req.notes != null) 
                        Text('Facility Notes: ${req.notes}', style: const TextStyle(color: MediColors.textSecondary, fontSize: 13, fontStyle: FontStyle.italic)),
                      ...
                      if (suggestion != null)
                        Container(
                          ...
                          margin: const EdgeInsets.only(bottom: 20),
                          decoration: BoxDecoration(
                            color: suggestion.contains('✅') ? MediColors.success.withValues(alpha: 0.1) : MediColors.warning.withValues(alpha: 0.1),
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(color: suggestion.contains('✅') ? MediColors.success.withValues(alpha: 0.3) : MediColors.warning.withValues(alpha: 0.3)),
                          ),
                          child: Text(suggestion, style: TextStyle(color: suggestion.contains('✅') ? MediColors.success : MediColors.warning, fontWeight: FontWeight.w500)),
                        ),

                      Row(
                        children: [
                          OutlinedButton.icon(
                            onPressed: isAiLoading ? null : () => _analyzeRequest(req),
                            icon: isAiLoading 
                              ? const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2))
                              : const Icon(Icons.auto_awesome, size: 16),
                            label: const Text('Analyze with AI'),
                            style: OutlinedButton.styleFrom(foregroundColor: MediColors.primaryLight),
                          ),
                          const Spacer(),
                          TextButton(
                            onPressed: _isActionInProgress ? null : () => _updateStatus(req.id, RequestStatus.rejected),
                            style: TextButton.styleFrom(foregroundColor: MediColors.error),
                            child: const Text('Decline'),
                          ),
                          const SizedBox(width: 12),
                          FilledButton(
                            onPressed: _isActionInProgress ? null : () => _updateStatus(req.id, RequestStatus.approved),
                            style: FilledButton.styleFrom(backgroundColor: MediColors.success),
